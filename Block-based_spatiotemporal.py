...
def param_search(param_grid, X, y, cv_func):
    """
    通用参数搜索框架
    cv_func: 接收 (param_dict, X, y) -> avg_val_rmse 的函数
    """
...
def run_space_cv(train_data, X, y, features, output_folder):
    ...
    # 导出测试集
    test_samples = train_data[test_idx].copy()
    test_path = os.path.join(output_folder, 'Space_独立测试集样本.xlsx')
    test_samples.to_excel(test_path, index=False)
    print(f"已导出独立测试集: {test_path}")
    ...
    for val_fold in range(5):
        ...
        best_val_pred_results.extend(list(zip(y_val_fold.tolist(), y_val_pred.tolist())))

    # 计算5折验证集整体RMSE
    all_val_true, all_val_pred = zip(*best_val_pred_results)
    final_val_rmse = np.sqrt(mean_squared_error(all_val_true, all_val_pred))
    ...
    print(f"最终模型 - 训练集RMSE: {train_rmse:.4f}")
    print(f"最终模型 - 5折验证集RMSE: {final_val_rmse:.4f}")
    print(f"最终模型 - 独立测试集RMSE: {test_rmse:.4f}")

    # 保存结果
    results_df = pd.DataFrame(all_results)
    final_results = pd.DataFrame({
        '评估数据集': ['训练集(回带)', '验证集(5折CV)', '独立测试集'],
        'RMSE': [train_rmse, final_val_rmse, test_rmse]
    })
...
def run_time_cv(train_data, X, y, features, output_folder):
    ...
    final_results = pd.DataFrame({
        ...
        'RMSE': [train_rmse, best_val_rmse, test_rmse]
    })
...
def run_random_cv(train_data, X, y, features, output_folder):
    ...
    print(f"最终模型 - 训练集RMSE: {train_rmse:.4f}")
    print(f"最终模型 - 验证集RMSE: {best_val_rmse:.4f}")
    print(f"最终模型 - 测试集RMSE: {test_rmse:.4f}")

    # 保存结果
    results_df = pd.DataFrame(all_results)
    final_results = pd.DataFrame({
        '评估数据集': ['训练集(回带)', '验证集(5折CV)', '测试集'],
        'RMSE': [train_rmse, best_val_rmse, test_rmse]
    })
...
#     output_folder_path = "E:\\huai_river\\Huairiver_GEE_data\\Daily_data\\old\\result_260624"
#
#     # 选择要运行的模型（可自由组合）
#     # 选项: 'Space', 'Time', 'Random'
#     # 示例:
#     #   run_models = ['Space']           # 只跑空间CV
#     #   run_models = ['Space', 'Time']     # 跑空间+时间
#     #   run_models = ['Space', 'Random']   # 跑空间+随机
#     #   run_models = ['Time', 'Random']    # 跑时间+随机
#     #   run_models = ['Space', 'Time', 'Random']  # 全部运行（默认）
#
#     run_models = ['Space']  # ← 修改这里选择模型
#
...
#     train_file_path = "E:\\huai_river\\Station_RWT\\Extracted_matching_with_temp.xlsx"
#     test_folder_path = 'E:\\huai_river\\Huairiver_GEE_data\\Daily_data\\old\\11-test-1'
#     output_folder_path = "E:\\huai_river\\Huairiver_GEE_data\\Daily_data\\old\\result_260625"
#
#     # 选择要运行的模型（可自由组合）
#     run_models = ['Space']  # ← 修改这里选择模型
#
...
if __name__ == "__main__":
    ...
    train_file_path = "E:\\huai_river\\Station_RWT\\Extracted_matching_with_temp.xlsx"
    test_folder_path = 'E:\\huai_river\\Huairiver_GEE_data\\Daily_data\\old\\11-test-1'
    output_folder_path = "E:\huai_river\Accuracy\\0608_overhaul\comment2\\1_result_4"

    # train_file_path = "E:\\huai_river\\Station_RWT\\Extracted_matching_with_temp_buchong.xlsx"
    # test_folder_path = 'E:\\huai_river\\Huairiver_GEE_data\\Daily_data\\old\\11-2'  #11-test-1
    # output_folder_path = "E:\huai_river\Accuracy\\0608_overhaul\comment2\\1_result_0702"
